...
message = 'PasteCheck by Wrath/Shinigami\n'
...
def main():
    banner(message)
    if len(sys.argv) < 1:
        print("Usage: python3 pastecheck.py <list>")
        sys.exit()
    elif len(sys.argv[1]):
        ulist = sys.argv[1]
        proxy = proxys()
        ...
        try:
            ...
            for username in content:
                res = requests.get("https://pastebin.com/u/" + username, headers=headers, proxies=proxy)

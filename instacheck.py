...
message = 'InstaCheck by Wrath/Shinigami\n'
...
def main():
    banner(message)
    if len(sys.argv) < 1:
        print("Usage: python3 instacheck.py <list>")
        sys.exit()
    elif len(sys.argv[1]):
        ulist = sys.argv[1]
        proxy = proxys()
        ...
        try:
            ...
            for username in content:
                res = requests.get("https://instagram.com/" + username, headers=headers, proxies=proxy)
